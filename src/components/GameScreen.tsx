...
import { Shield, Wrench, ShoppingBag, Globe, Lock, History, ChevronRight, Zap, Anchor, Waves } from 'lucide-react';

interface GameScreenProps {
  openModal: (modal: 'upgrades' | 'shop' | 'server' | 'repair' | 'raids' | 'attack' | 'shipInspect') => void;
  onSelectTargetForAttack?: (player: Player) => void;
}

export const GameScreen: React.FC<GameScreenProps> = ({ openModal, onSelectTargetForAttack }) => {
  const {
    energy,
    currentServer,
    shipCondition,
  ...
  return (
    <div className="relative h-full flex-1 p-2 sm:p-3 flex flex-col justify-between overflow-hidden select-none">
      ...
      <div className="relative z-10 max-w-5xl mx-auto w-full flex flex-col space-y-3 my-auto">
        ...
        <div className="flex items-center justify-center gap-2 max-w-md mx-auto w-full bg-[#451a03]/90 border-4 border-[#78350f] rounded-2xl p-1.5 shadow-2xl backdrop-blur-md">
          <button
        ...
        {activeTab === 'build' ? (
          <div className="flex flex-col items-center justify-center space-y-4 animate-fade-in w-full">
            ...
            <div className="w-full max-w-md bg-[#78350f]/90 border-4 border-[#451a03] rounded-2xl p-2.5 text-amber-100 shadow-xl backdrop-blur-sm flex items-center justify-between">
              <div className="flex items-center gap-2">
                {currentServer.type === 'global' ? (
                  ...
                  <div className="p-1.5 bg-[#451a03] border border-[#b45309] rounded-lg text-[#facc15]">
            ...
            <ShipDisplay onInspectShip={() => openModal('shipInspect')} isHugeBuildMode={true} />
            ...
            <div className="w-full max-w-md space-y-2">
              ...
              {shipCondition <= 50 && (
                <div className="text-[11px] font-bold text-center text-red-200 bg-red-950/90 border border-red-700 p-2 rounded-xl shadow-lg">
                  ⚠️ Hull damaged ({shipCondition}%). Use Repair in the Shipyard below before setting sail!
                </div>
              )}
              ...
              <div className="grid grid-cols-4 gap-1.5 sm:gap-2">
                <button
                  ...
                  className="bg-[#b45309] hover:bg-[#d97706] active:translate-y-0.5 border-b-2 sm:border-b-4 border-r-2 border-[#451a03] py-2 sm:py-2.5 px-1 sm:px-2 rounded-xl text-center flex flex-col items-center justify-center gap-1 text-white shadow-xl italic font-black uppercase"
                >
                  <Shield className="w-4 h-4 sm:w-5 sm:h-5 text-[#facc15]" />
                  <span className="text-[9px] sm:text-[10px] truncate w-full">Upgrades</span>
                </button>

                <button
                  onClick={() => openModal('repair')}
                  className="bg-[#16a34a] hover:bg-[#22c55e] active:translate-y-0.5 border-b-2 sm:border-b-4 border-r-2 border-[#064e3b] py-2 sm:py-2.5 px-1 sm:px-2 rounded-xl text-center flex flex-col items-center justify-center gap-1 text-white shadow-xl italic font-black uppercase"
                >
                  <Wrench className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
                  <span className="text-[9px] sm:text-[10px] truncate w-full">Repair</span>
                </button>

                <button
                  onClick={() => openModal('shop')}
                  className="bg-[#1d4ed8] hover:bg-[#2563eb] active:translate-y-0.5 border-b-2 sm:border-b-4 border-r-2 border-[#1e3a8a] py-2 sm:py-2.5 px-1 sm:px-2 rounded-xl text-center flex flex-col items-center justify-center gap-1 text-white shadow-xl italic font-black uppercase"
                >
                  <ShoppingBag className="w-4 h-4 sm:w-5 sm:h-5 text-sky-300" />
                  <span className="text-[9px] sm:text-[10px] truncate w-full">Shop</span>
                </button>

                <button
                  onClick={() => openModal('raids')}
                  className="bg-[#451a03] hover:bg-[#78350f] active:translate-y-0.5 border-b-2 sm:border-b-4 border-r-2 border-[#1c0a02] py-2 sm:py-2.5 px-1 sm:px-2 rounded-xl text-center flex flex-col items-center justify-center gap-1 text-[#fde68a] shadow-xl relative italic font-black uppercase"
                >
                  <History className="w-4 h-4 sm:w-5 sm:h-5 text-[#facc15]" />
                  <span className="text-[9px] sm:text-[10px] truncate w-full">Raid Log</span>
                  {raidLogs.length > 0 && (
