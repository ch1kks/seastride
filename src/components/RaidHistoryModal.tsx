...
export const RaidHistoryModal: React.FC<RaidHistoryModalProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-3 select-none">
      <div className="bg-[#78350f] border-8 border-[#451a03] rounded-3xl w-full max-w-md overflow-hidden shadow-2xl relative text-amber-100 flex flex-col max-h-[85vh]">
        ...
        <div className="p-4 overflow-y-auto space-y-2 flex-1">
          {raidLogs.length === 0 ? (
            ...
            raidLogs.map(log => (
              <div
                key={log.id}
                className="bg-[#451a03] border-2 border-[#b45309] rounded-2xl p-3 flex items-center justify-between gap-2"
              >
                <div className="flex items-center gap-3">
                  ...
                  >
                    {log.type === 'attack' ? <Crosshair className="w-4 h-4" /> : <ShieldAlert className="w-4 h-4" />}
                  </div>

                  <div>
                    <div className="text-xs font-black text-white font-serif">
                      {log.type === 'attack' ? `Raid on ${log.opponentName}` : `Defended against ${log.opponentName}`}
                    </div>
