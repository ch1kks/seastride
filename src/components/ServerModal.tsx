...
export const ServerModal: React.FC<ServerModalProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-3 select-none">
      <div className="bg-[#78350f] border-8 border-[#451a03] rounded-3xl w-full max-w-md overflow-hidden shadow-2xl relative text-amber-100 flex flex-col max-h-[90vh]">
        ...
        <div className="p-4 overflow-y-auto space-y-4 flex-1">
          ...
          <div className="bg-[#451a03] border-4 border-[#b45309] rounded-2xl p-3 text-center space-y-1">
                ...
                return (
                  <div
                    ...
                    className={`p-3 rounded-xl border-2 flex items-center justify-between cursor-pointer transition-all ${
                      isCurrent
                        ? 'bg-[#451a03] border-[#facc15] shadow-lg'
                        : 'bg-[#451a03] border-[#b45309] hover:border-[#fde68a]'
                    }`}
                  >
                    <div className="flex items-center gap-2.5">
                      {s.type === 'global' ? (
                        ...
                        <div className="p-2 bg-[#78350f] border-2 border-[#b45309] rounded-lg text-[#fbbf24]">
                    ...
                    <div>
                      {isCurrent ? (
                        <span className="text-xs font-black text-white flex items-center gap-1 bg-[#16a34a] px-2.5 py-1 rounded-lg border border-[#064e3b]">
                          <Check className="w-3.5 h-3.5 text-white" /> Active
                        </span>
                      ) : (
                        <span className="text-xs font-bold text-[#fde68a] bg-[#78350f] px-2.5 py-1 rounded-lg border border-[#b45309]">
                          Switch
                        </span>
                      )}
                    </div>
          ...
          {/* Join By Code */}
          <form onSubmit={handleJoinCustomCode} className="bg-[#451a03] border-2 border-[#b45309] p-3.5 rounded-2xl space-y-2">
            <span className="text-xs font-serif font-black uppercase text-[#fde68a]">
              Enter Server Code
            </span>
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-[#1c0a02] border-2 border-[#78350f] rounded-xl px-3 py-1.5 text-xs text-white uppercase font-mono tracking-wider focus:outline-none focus:border-[#fbbf24]"
              />
              <button
                type="submit"
          ...
          <form onSubmit={handleCreateServer} className="bg-[#451a03] border-4 border-[#b45309] p-3.5 rounded-2xl space-y-2">
            ...
            <input
              ...
              className="w-full bg-[#1c0a02] border-2 border-[#78350f] rounded-xl px-3 py-1.5 text-xs text-white focus:outline-none focus:border-[#fbbf24]"
            />
